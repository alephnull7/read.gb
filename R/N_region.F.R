...
function(Feat, SQuali, SQualiN){
  ...
  N_region <- lapply(N_region, function(x){gsub(" {2,}", " ", x, perl = TRUE)})
  N_region <- lapply(N_region, function(x){gsub("\"", "", x, fixed = T)})
  N_region <- lapply(N_region, function(x){gsub("\\", "", x, fixed = T)})
  N_region <- lapply(N_region, function(x){gsub("[^[:alnum:][:space:][]'.,:_<>()-]", "", x, perl = TRUE)})
  return(N_region)
}
