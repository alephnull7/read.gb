...
function(Feat, SQuali, SQualiN){
  ...
  misc_feature <- lapply(misc_feature, function(x){gsub(" {2,}", " ", x, perl = TRUE)})
  misc_feature <- lapply(misc_feature, function(x){gsub("\"", "", x, fixed = T)})
  misc_feature <- lapply(misc_feature, function(x){gsub("\\", "", x, fixed = T)})
  misc_feature <- lapply(misc_feature, function(x){gsub("[^[:alnum:][:space:][]'.,:_<>()-]", "", x, perl = TRUE)})
  return(misc_feature)
}
