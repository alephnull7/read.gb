...
function(Feat, SQuali, SQualiN){
  ...
  ncRNA <- lapply(ncRNA, function(x){gsub(" {2,}", " ", x, perl = TRUE)})
  ncRNA <- lapply(ncRNA, function(x){gsub("\"", "", x, fixed = T)})
  ncRNA <- lapply(ncRNA, function(x){gsub("\\", "", x, fixed = T)})
  ncRNA <- lapply(ncRNA, function(x){gsub("[^[:alnum:][:space:][]'.,:_<>()-]", "", x, perl = TRUE)})
  return(ncRNA)
}
