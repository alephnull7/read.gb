...
function(Feat, SQuali, SQualiN){
  ...
  misc_RNA <- lapply(misc_RNA, function(x){gsub(" {2,}", " ", x, perl = TRUE)})
  misc_RNA <- lapply(misc_RNA, function(x){gsub("\"", "", x, fixed = T)})
  misc_RNA <- lapply(misc_RNA, function(x){gsub("\\", "", x, fixed = T)})
  misc_RNA <- lapply(misc_RNA, function(x){gsub("[^[:alnum:][:space:][]'.,:_<>()-]", "", x, perl = TRUE)})
  return(misc_RNA)
}
