...
function(Feat, SQuali, SQualiN){
  ...
  source <- lapply(source, function(x){gsub(" {2,}", " ", x, perl = TRUE)})
  source <- lapply(source, function(x){gsub("\"", "", x, fixed = T)})
  source <- lapply(source, function(x){gsub("\\", "", x, fixed = T)})
  source <- lapply(source, function(x){gsub("[^[:alnum:][:space:][]'.,:_<>()-]", "", x, perl = TRUE)})
  return(source)
}
